...
export default function ConfigsTab() {
  ...
  const filteredConfigs = configs.filter((c) => {
    ...
    return c.serviceType.toLowerCase().includes(query) || label.toLowerCase().includes(query);
  });

  const { register, handleSubmit, reset, setValue, watch } = useForm<ConfigFormValues>({
    defaultValues: {
      serviceType: '',
      officialFee: 0,
      serviceFee: 0,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              filteredConfigs.map((c) => (
                <tr key={c.id}>
                  ...
                  <td style={{ textAlign: 'right', fontWeight: 600 }}>
                    ₹{Number(c.officialFee).toLocaleString('en-IN')}
                  </td>
                  <td style={{ textAlign: 'right', fontWeight: 600 }}>
                    ₹{Number(c.serviceFee).toLocaleString('en-IN')}
                  </td>
                  <td style={{ textAlign: 'right', fontWeight: 600 }}>
                    ₹{Number(c.protocolFee || 0).toLocaleString('en-IN')}
                  </td>
                  <td style={{ textAlign: 'right', fontWeight: 600 }}>
                    ₹{Number(c.defaultMiscFee || 0).toLocaleString('en-IN')}
                  </td>
                  <td style={{ textAlign: 'center' }}>
                    <span
                      className={`badge ${c.allowManualOverride ? 'badge-success' : 'badge-danger'}`}
                    >
      ...
      {isFormOpen && (
        <Modal
          title={editingConfigId ? 'Edit Configuration' : 'Add Configuration'}
          onClose={closeForm}
        >
          <form
