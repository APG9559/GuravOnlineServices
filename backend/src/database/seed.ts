...
const SEED_USERS = [
  {
    name: 'Admin',
    email: 'admin@familystore.local',
    password: 'Admin@1234',
    role: Role.ADMIN,
  },
  {
    name: 'Operator',
    email: 'operator@familystore.local',
...
const dataSource = new DataSource({
  ...
  entities: [User, Affidavit, Marriage, PricingSetting],
  synchronize: true,
});

async function seed() {
  console.log('\n🌱  Family Store — Database Seeder');
  console.log('────────────────────────────────────');
  ...
  for (const seedUser of SEED_USERS) {
    ...
    if (existing) {
      console.log(`ℹ️   [${seedUser.role}] ${seedUser.email} already exists — skipping.`);
      continue;
    }
  ...
  for (const def of DEFAULT_PRICING) {
    ...
    if (exists) {
      console.log(`ℹ️   Pricing key "${def.key}" already exists — skipping.`);
      continue;
    }
  ...
  for (const p of DEFAULT_PRICING) {
    console.log(`  ${p.label.padEnd(30)} ₹${p.value}`);
  }
