    ...
    <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
      ...
      {renewalLoading ? (
        ...
        <div style={{ padding: 20, color: 'var(--text-muted)' }}>
          No businesses due for renewal at this time (the queue is active only during March and
          April).
        </div>
      ) : (
        <table>
                  ...
                  {(() => {
                    ...
                    if (stored) {
                      try {
                        ...
                        const found = parsed.find((t: { id: string; body?: string }) => t.id === 'tl_renewal_reminder');
                        if (found) {
                    ...
                    const handleSend = (channel: 'whatsapp' | 'sms') => {
                      messageLogsApi
                        .create({
                          ...
                          recipientPhone: primaryCustomer.phone
                            .replace(/^\+91/, '')
                            .replace(/\D/g, ''),
                          messageBody: msg,
                          recordId: b.id,
