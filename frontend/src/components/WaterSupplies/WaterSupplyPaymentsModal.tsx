...
import { WaterServiceRecord, PAYMENT_MODES } from '@/types';
...
interface WaterSupplyPaymentsModalProps {
...
export const WATER_SERVICE_TYPE_LABELS: Record<string, string> = {
  ...
  MeterDisconnection: 'Meter Disconnection',
  MeterReconnection: 'Meter Reconnection',
  ChangeOfUse: 'Change of Usage',
  MeterInspection: 'Meter Inspection',
  NoDuesCertificate: 'No Dues Certificate',
};

export default function WaterSupplyPaymentsModal({
  record: initialRecord,
  onClose,
}: WaterSupplyPaymentsModalProps) {
  ...
  const addPaymentMut = useMutation({
    mutationFn: (data: any) => waterSuppliesApi.addPayment(record.id, data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['water-record', record.id] });
      qc.invalidateQueries({ queryKey: ['water-records'] });
      qc.invalidateQueries({ queryKey: ['waterPayments'] });
      setAmount('');
      resetAccounts();
  ...
  const deletePaymentMut = useMutation({
    mutationFn: (id: string) => waterSuppliesApi.deletePayment(id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['water-record', record.id] });
      qc.invalidateQueries({ queryKey: ['water-records'] });
      qc.invalidateQueries({ queryKey: ['waterPayments'] });
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      <div className="card modal-card" style={{ width: '100%', maxWidth: 540, position: 'relative', padding: '1.5rem 2rem', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <h3 style={{ fontSize: 18, fontWeight: 700, marginBottom: '1.5rem', textAlign: 'center' }}>
          Payments — {record.connection?.currentOwner || 'Water Supply'}
        </h3>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px 16px', marginBottom: '1.5rem', fontSize: '13px' }}>
          <div>
            ...
            <span className="badge badge-blue">{WATER_SERVICE_TYPE_LABELS[record.serviceType] || record.serviceType}</span>
          </div>
          <div>
            <span style={{ color: 'var(--text-muted)', display: 'block' }}>Token No</span>
            <span style={{ fontWeight: 500 }}>{record.applicationTokenNo || '—'}</span>
          </div>
          <div>
          ...
          <div>
            ...
            <span style={{ fontWeight: 500 }}>{record.connection?.currentOwner || '—'}</span>
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: '11px', fontWeight: 700, color: 'var(--text-muted)', marginBottom: 4, textTransform: 'uppercase' }}>Total Cost</div>
            <div style={{ fontSize: '18px', fontWeight: 800 }}>₹{amountCharged.toLocaleString('en-IN')}</div>
          </div>
          <div style={{
            textAlign: 'center',
            padding: '12px 8px',
            ...
            <div style={{ fontSize: '11px', fontWeight: 700, color: 'var(--text-muted)', marginBottom: 4, textTransform: 'uppercase' }}>Paid</div>
            <div style={{ fontSize: '18px', fontWeight: 800 }}>₹{totalPaid.toLocaleString('en-IN')}</div>
          </div>
          <div style={{
            textAlign: 'center',
            padding: '12px 8px',
            ...
            <div style={{ fontSize: '11px', fontWeight: 700, color: 'var(--text-muted)', marginBottom: 4, textTransform: 'uppercase' }}>Balance</div>
            <div style={{ fontSize: '18px', fontWeight: 800 }}>₹{balance.toLocaleString('en-IN')}</div>
          </div>
        </div>
        ...
        <div style={{ marginBottom: '1rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <span style={{ fontSize: 14, fontWeight: 700, textTransform: 'uppercase' }}>Payment History</span>
            <button
          ...
          {payments.length === 0 && !showAddForm ? (
            <div style={{ padding: '20px', textAlign: 'center', color: 'var(--text-muted)', fontSize: 13, border: '2px dashed var(--border-light)', borderRadius: 'var(--radius)' }}>
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                      <span style={{ fontWeight: 700, fontSize: 15 }}>₹{Number(p.amount).toLocaleString('en-IN')}</span>
                      <span className="badge badge-blue" style={{ fontSize: 10 }}>{p.paymentMode}</span>
        ...
        {/* Add Payment Form */}
        {showAddForm && (
          <form onSubmit={handleAddPaymentSubmit} style={{ border: '2.5px solid var(--border)', borderRadius: 'var(--radius)', padding: '16px', background: 'var(--surface)', boxShadow: '3px 3px 0px var(--border)' }}>
            <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 12, textTransform: 'uppercase' }}>Record Payment</div>
            ...
            {paymentMode === 'UPI' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '12px' }}>
                ...
                <UpiQrCode
                  ...
                  transactionRef={`${record.applicationTokenNo || record.id}-${Date.now()}`}
                  transactionNote={`Payment for Water Supply ${record.applicationTokenNo || record.id}`}
                />
              </div>
            )}
