...
export default function UserPerformanceTable({
  ...
  const [selectedOperator, setSelectedOperator] = useState<{ userId: string; userName: string } | null>(null);
  ...
  return (
    <div className="card" style={{ marginTop: '1.5rem', padding: 0, overflow: 'hidden' }}>
      ...
      {selectedOperator && (
        ...
        >
          <div style={{ minHeight: isCalendarOpen ? '420px' : 'auto', transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)', display: 'flex', flexDirection: 'column' }}>
            ...
            <div className="form-group" style={{ marginBottom: 0, flex: 1, minWidth: 130 }}>
              <label style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 4 }}>From Date</label>
              <NeoDatePicker
                value={localFromDate}
            ...
            <div className="form-group" style={{ marginBottom: 0, flex: 1, minWidth: 130 }}>
              <label style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 4 }}>To Date</label>
              <NeoDatePicker
                value={localToDate}
            ...
            <button
              className="btn btn-secondary btn-sm"
              style={{ height: 32, padding: '0 12px', fontSize: 11, display: 'flex', alignItems: 'center' }}
              onClick={() => {
                setLocalFromDate(fromDate || '');
                setLocalToDate(toDate || '');
              }}
          ...
          {expensesLoading ? (
            ...
            <div className="table-wrapper" style={{ maxHeight: '50vh', overflowY: 'auto' }}>
              <table style={{ margin: 0, width: '100%' }}>
                ...
                <tbody>
                  {expenses.map((exp: any) => (
                    <tr key={exp.id}>
                      ...
                      <td>
                        <span className={`badge ${exp.category === 'Shop' ? 'badge-blue' : 'badge-amber'}`}>
                          {exp.category}
                        </span>
                      </td>
                      <td style={{ fontWeight: 600 }}>{exp.type}</td>
                      <td style={{ color: 'var(--text-muted)', fontSize: 13, maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={exp.description || ''}>
                        {exp.description || <span style={{ fontStyle: 'italic', color: '#ccc' }}>—</span>}
                      </td>
                      <td style={{ textAlign: 'right', fontWeight: 700, color: 'rgb(220, 38, 38)' }}>
                ...
                {expenses.length > 0 && (
                  <tfoot>
                    <tr style={{ borderTop: '2px solid var(--border)', background: 'var(--card-bg)' }}>
                      <td colSpan={4} style={{ fontWeight: 800, textAlign: 'right', padding: '12px 16px' }}>
                        Total:
                      </td>
                      <td style={{ fontWeight: 900, textAlign: 'right', color: 'rgb(220, 38, 38)', padding: '12px 16px' }}>
