  ...
  return (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <h3
            style={{ fontSize: 16, fontWeight: 500, display: 'flex', alignItems: 'center', gap: 8 }}
          >
            👤 {detailsLoading ? 'Loading profile...' : customerDetails?.name}
          </h3>
          <p style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>
            Registered on{' '}
            {customerDetails && new Date(customerDetails.createdAt).toLocaleDateString('en-IN')}
          </p>
        </div>
      ...
      {detailsLoading ? (
        ...
        <>
          ...
          <div
            className="grid-2"
            style={{ background: 'var(--bg)', padding: '0.75rem', borderRadius: 'var(--radius)' }}
          >
            <div>
          ...
          <div style={{ flex: 1, minHeight: 0 }}>
            ...
            >
              ...
              <span className="badge badge-blue">
                {customerDetails.services.length} services availed
              </span>
            </h4>
                ...
                >
                    ...
                    <span
                      className={`badge ${
                        s.type === 'marriage'
                          ? 'badge-amber'
                          : s.type === 'birth-death' ||
                              s.type === 'trade-license' ||
                              s.type === 'water-supply' ||
                              s.type === 'property-tax'
                            ? 'badge-green'
                            : s.type === 'pan-card' || s.type === 'passport'
                              ? 'badge-red'
                              : 'badge-blue'
                      }`}
                      style={{ fontSize: 11 }}
                  ...
                  >
                    <div>
                      ...
                      <span style={{ color: 'var(--text-hint)', fontSize: 11, marginLeft: 6 }}>
                        by {s.createdBy}
                      </span>
                    </div>
                    <div style={{ display: 'flex', gap: 6 }}>
