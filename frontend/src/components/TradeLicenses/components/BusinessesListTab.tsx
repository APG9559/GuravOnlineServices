...
import { Business, BusinessTrade, Customer, TradeLicenseRecord } from '@/types';
...
export default function BusinessesListTab({ startServiceForBusiness }: BusinessesListTabProps) {
  ...
  const { data: viewingBusinessDetails, isLoading: detailsLoading } = useQuery({
    ...
    queryFn: () =>
      viewingBusinessId
        ? tradeLicensesApi.getBusinessDetails(viewingBusinessId).then((r) => r.data)
        : null,
    enabled: !!viewingBusinessId,
  });

  const verifyCertificateMutation = useMutation({
    mutationFn: ({ businessId, data }: { businessId: string; data: { status: string; verificationStatus: string; submittedAt?: string; verifiedAt?: string } }) =>
      ...
      <div className="card">
        {detailsLoading ? (
          ...
          <div>
              ...
              <div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 4 }}>
                  {viewingBusinessDetails.trades && viewingBusinessDetails.trades.length > 0
                    ? viewingBusinessDetails.trades.map((t: BusinessTrade, i: number) => (
            ...
            >
              ...
              >
                <div>
                  ...
                  <span
                    className={`badge ${viewingBusinessDetails.completionCertificateStatus === 'Available' ? 'badge-green' : 'badge-danger'}`}
                  >
                  ...
                  <span
                    className={`badge ${
                      viewingBusinessDetails.completionCertificateVerificationStatus === 'Verified'
                        ? 'badge-green'
                        : viewingBusinessDetails.completionCertificateVerificationStatus ===
                            'Pending'
                          ? 'badge-amber'
                          : 'badge-danger'
                    }`}
            ...
            <hr className="divider" style={{ margin: '1rem 0' }} />
            <div style={{ fontWeight: 500, fontSize: 14, marginBottom: 8 }}>
              Tenant & Security Deposit Details
            </div>
            <div
              style={{
                padding: 12,
            ...
            >
              <div className="grid-2" style={{ gap: '8px 16px' }}>
                <div>
                  ...
                  <span
                    className={`badge ${viewingBusinessDetails.isTenant ? 'badge-amber' : 'badge-green'}`}
                  >
                ...
                <div>
                  ...
                  <span
                    className={`badge ${viewingBusinessDetails.depositFeeCharged ? 'badge-green' : 'badge-secondary'}`}
                  >
              ...
              <div className="grid-2" style={{ gap: '8px 16px' }}>
                ...
                <div>
                  ...
                  {viewingBusinessDetails.depositFeeCollectionDate
                    ? new Date(viewingBusinessDetails.depositFeeCollectionDate).toLocaleDateString(
                        'en-IN',
                      )
                    : '—'}
                </div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: '1.25rem' }}>
              {viewingBusinessDetails.customers?.map((c: Customer) => (
                <span
                  key={c.id}
            ...
            {viewingBusinessDetails.records?.length === 0 ? (
              ...
              >
                <table style={{ margin: 0, fontSize: 12 }}>
                  ...
                  <tbody>
                    {viewingBusinessDetails.records?.map((rec: TradeLicenseRecord) => (
                      <tr key={rec.id}>
                        ...
                        <td style={{ fontWeight: 500 }}>
                          ₹{Number(rec.amountCharged).toLocaleString('en-IN')}
                        </td>
              ...
              <>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  ...
                  <button
                    className="btn btn-sm"
                    onClick={() =>
                      startServiceForBusiness(viewingBusinessDetails, 'Transfer_Third_Party')
                    }
                  >
                  ...
                  <button
                    className="btn btn-sm"
                    onClick={() =>
                      startServiceForBusiness(viewingBusinessDetails, 'Partner_Change')
                    }
                  >
                    Amend Partners
                  </button>
