...
export default function ServiceLogsTab({ onPrint }: ServiceLogsTabProps) {
  ...
  const [selectedRecordForPayments, setSelectedRecordForPayments] = useState<WaterServiceRecord | null>(null);
  const [selectedRecordForDocuments, setSelectedRecordForDocuments] = useState<WaterServiceRecord | null>(null);
              ...
              records.map((r: WaterServiceRecord) => {
                ...
                return (
                  <tr key={r.id}>
                    ...
                    <td>
                      ...
                      {r.applicationTokenNo && (
                        <div
                          ...
                          style={{ fontSize: 11, color: 'var(--text-muted)', cursor: 'pointer', marginTop: 4, display: 'inline-block' }}
                          title="Click to copy token number"
                        >
                          Token: <span style={{ textDecoration: 'underline' }}>{r.applicationTokenNo}</span> 📋
                        </div>
                      )}
                    </td>
                    ...
                    <td style={{ textAlign: 'right', fontWeight: 600 }}>₹{totalCost.toLocaleString('en-IN')}</td>
                    <td style={{ textAlign: 'right', fontWeight: 600, color: 'var(--success)' }}>₹{totalPaid.toLocaleString('en-IN')}</td>
                    <td style={{ textAlign: 'right', fontWeight: 700, color: balance > 0 ? 'var(--danger)' : 'var(--success)' }}>
