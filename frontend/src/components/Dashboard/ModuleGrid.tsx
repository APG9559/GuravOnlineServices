          ...
          >
            ...
            >
              <div>
                ...
                <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--text)' }}>
                  ₹{m.grossEarnings.toLocaleString('en-IN')}
                </div>
              </div>
              <div>
                <div
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {Object.entries(m.subServices).map(([subKey, sub]: [string, { label: string; count: number; grossEarnings: number; netEarnings: number }]) => (
                  <div
                    key={subKey}
                    style={{
                      display: 'flex',
                  ...
                  >
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontWeight: 700, fontSize: 12, color: 'var(--text)' }}>
                        ₹{sub.grossEarnings.toLocaleString('en-IN')}{' '}
                        <span
                          style={{ fontSize: 9, color: 'var(--text-muted)', fontWeight: 'normal' }}
                        >
                          g
                        </span>
                      </div>
                      <div style={{ fontWeight: 700, fontSize: 11, color: 'var(--text-muted)' }}>
                        ₹{sub.netEarnings.toLocaleString('en-IN')}{' '}
                        <span style={{ fontSize: 9, fontWeight: 'normal' }}>n</span>
                      </div>
                    </div>
                  </div>
                ))}
