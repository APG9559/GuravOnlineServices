...
export default function PropertyTaxPage() {
  const [savedRecord, setSavedRecord] = useState<PropertyTax | null>(null);
  ...
  const getPricingKeys = (type: string) => {
    switch (type) {
      case 'AssessmentCopy':
        return { official: 'property_tax_assessment_official_fee', service: 'property_tax_assessment_service_fee', protocol: 'property_tax_assessment_protocol_fee' };
      case 'NameTransfer':
        return { official: 'property_tax_transfer_official_fee', service: 'property_tax_transfer_service_fee', protocol: 'property_tax_transfer_protocol_fee' };
      case 'NoDuesCertificate':
        return { official: 'property_tax_nodues_official_fee', service: 'property_tax_nodues_service_fee', protocol: 'property_tax_nodues_protocol_fee' };
      default:
        return { official: 'property_tax_assessment_official_fee', service: 'property_tax_assessment_service_fee', protocol: 'property_tax_assessment_protocol_fee' };
    }
  };
  ...
  const getFallbackFees = (type: string) => {
    switch (type) {
      case 'AssessmentCopy': return { official: 200, service: 150, protocol: 50 };
      case 'NameTransfer': return { official: 500, service: 300, protocol: 100 };
      case 'NoDuesCertificate': return { official: 150, service: 100, protocol: 50 };
      default: return { official: 200, service: 150, protocol: 50 };
    }
  };
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      reset({
        ...
        amountCharged: (pricing.property_tax_assessment_official_fee ?? 200) + (pricing.property_tax_assessment_service_fee ?? 150) + (pricing.property_tax_assessment_protocol_fee ?? 50),
      });
    },
  });
  ...
  return (
    <div>
      ...
      <div className="card" style={{ maxWidth: 800 }}>
        <div style={{ fontWeight: 500, marginBottom: '1.5rem', fontSize: '1.1rem' }}>New Property Tax Application Record</div>
        ...
        <form onSubmit={handleSubmit((d) => mutation.mutate(d))}>
          ...
          <div className="form-group" style={{ marginTop: 16 }}>
            ...
            />
            {errors.address && <span style={{ color: 'var(--danger)', fontSize: 12 }}>Required</span>}
          </div>
          ...
          <div className="grid-2" style={{ marginTop: 16 }}>
            <div className="form-group">
              ...
              />
              {errors.propertyTaxNo && <span style={{ color: 'var(--danger)', fontSize: 12 }}>Required</span>}
            </div>
        ...
        <div className="modal-backdrop">
          <div className="modal-content" style={{ maxWidth: 600 }}>
            <div style={{ textAlign: 'center', marginBottom: 20 }}>
              ...
              <h2 style={{ fontSize: '1.5rem', fontWeight: 'bold', margin: '10px 0' }}>Record Saved Successfully!</h2>
              <p style={{ color: 'var(--text-muted)' }}>The Property Tax application has been stored.</p>
            </div>
