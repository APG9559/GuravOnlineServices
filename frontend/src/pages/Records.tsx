...
const EXPORT_MAPPERS: ExportMappersMap = {
  affidavits: {
...
const RECEIPT_MAP: Record<SubTab, React.ComponentType<{ record: never }>> = {
  affidavits: AffidavitReceipt as never,
  marriages: MarriageReceipt as never,
  birthDeath: BirthDeathReceipt as never,
  propertyCards: PropertyCardReceipt as never,
  shopAct: ShopActLicenseReceipt as never,
  tradeLicenses: TradeLicenseReceipt as never,
  panCards: PanCardReceipt as never,
  passports: PassportReceipt as never,
  voterCards: VoterCardReceipt as never,
  gazettes: GazetteReceipt as never,
  waterSupplies: WaterSupplyReceipt as never,
  propertyTaxes: PropertyTaxReceipt as never,
};

const API_MAP: Record<SubTab, { getAll: (params?: Record<string, string>) => Promise<{ data: unknown }>; update: (id: string, data: unknown) => Promise<unknown>; delete: (id: string) => Promise<unknown> }> = {
...
export default function RecordsPage() {
  ...
  const exportCurrent = async () => {
    ...
    try {
      ...
      toast.error(e.response?.data?.message || e.message || 'Failed to export records.');
    }
  };
