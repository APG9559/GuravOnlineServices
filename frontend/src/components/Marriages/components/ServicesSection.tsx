              ...
              <input
                ...
                onChange={(e) => {
                  const next = e.target.checked
                    ? [...watchSvcs, s.key]
                    : watchSvcs.filter((x: string) => x !== s.key);
      ...
      <div key="consultancy" style={{ marginBottom: 12 }}>
        <div className="checkbox-row" style={{ marginBottom: 0 }}>
          <input
            ...
            id="f-consultancy"
            checked={
              watchSvcs.includes('Marriage Registration Consultancy Fee') ||
              watchSvcs.includes('Marriage Consultancy Fee')
            }
            onChange={(e) => {
              const next = e.target.checked
                ? [
                    ...watchSvcs.filter((x: string) => x !== 'Marriage Consultancy Fee'),
                    'Marriage Registration Consultancy Fee',
                  ]
                : watchSvcs.filter(
                    (x: string) =>
                      x !== 'Marriage Consultancy Fee' &&
                      x !== 'Marriage Registration Consultancy Fee',
                  );
              setValue('servicesProvided', next);
              if (e.target.checked) {
