...
import { WATER_SERVICE_TYPE_LABELS } from './WaterSupplyPaymentsModal';
import { useAuth } from '@/context/AuthContext';

interface ConnectionsListTabProps {
  startServiceForConnection: (connection: WaterConnection, serviceType: WaterServiceRecord['serviceType']) => void;
}
...
export default function ConnectionsListTab({ startServiceForConnection }: ConnectionsListTabProps) {
  ...
  const approveMutation = useMutation({
    ...
    onError: (err: any) => {
      setApproveError(err.response?.data?.message || err.message || 'Failed to approve connection');
    },
  });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <div style={{ border: '3.5px solid var(--border)', borderRadius: 'var(--radius)', boxShadow: '4px 4px 0 var(--border)', overflow: 'visible', position: 'relative' }}>
        <div className="table-responsive" style={{ borderRadius: 'calc(var(--radius) - 3px)', overflow: 'hidden' }}>
            ...
            {connections.length === 0 ? (
              ...
              connections.map((c) => (
                <tr key={c.id}>
                  ...
                  <td>
                    <div style={{ maxWidth: 200, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }} title={c.connectionAddress}>
                      {c.connectionAddress}
                    </div>
                  </td>
                  <td>
                    <div style={{ fontSize: 12 }}>
                      ...
                      {c.contactPersonPhone && <div style={{ color: 'var(--text-muted)' }}>📞 {c.contactPersonPhone}</div>}
                    </div>
                  </td>
                    ...
                    <ActionsDropdown
                      items={[
                        ...
                        ...(c.connectionStatus === 'Pending' && hasApproveAccess ? [{
                          ...
                          dividerBefore: true,
                          onClick: () => { setApprovingConnection(c); setConnectionNo(''); },
                        }] : []),
                        ...(c.connectionStatus === 'Active' ? [
                          { label: 'Transfer Ownership', icon: '🔁', dividerBefore: true, onClick: () => startServiceForConnection(c, 'ConnectionTransfer') },
                          { label: 'Change Usage Type', icon: '🔄', onClick: () => startServiceForConnection(c, 'ChangeOfUse') },
                          { label: 'Disconnect Meter',  icon: '🔌', onClick: () => startServiceForConnection(c, 'MeterDisconnection'), variant: 'danger' as const },
                          { label: 'Request Inspection', icon: '🔍', onClick: () => startServiceForConnection(c, 'MeterInspection') },
                          { label: 'No Dues Certificate', icon: '📄', onClick: () => startServiceForConnection(c, 'NoDuesCertificate') },
                        ] : []),
      ...
      {approvingConnection && (
        <Modal title="Approve Water Connection" onClose={() => setApprovingConnection(null)}>
          <form onSubmit={handleApproveSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div>
              <span style={{ color: 'var(--text-muted)', display: 'block', fontSize: 12 }}>Owner Name</span>
              <span style={{ fontWeight: 700 }}>{approvingConnection.currentOwner}</span>
            </div>
            <div>
              <span style={{ color: 'var(--text-muted)', display: 'block', fontSize: 12 }}>Address</span>
      ...
      {viewingConnection && (
        <Modal title={`Connection Profile — ${viewingConnection.connectionNo || 'PENDING'}`} onClose={() => setViewingConnection(null)}>
          {detailsLoading ? (
            <div style={{ padding: '2rem', textAlign: 'center' }}>Loading profile details...</div>
          ) : connectionDetails ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              {/* Profile Details Grid */}
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', borderBottom: '2.5px solid var(--border)', paddingBottom: '16px' }}>
                <div>
                  <span style={{ color: 'var(--text-muted)', display: 'block', fontSize: 12 }}>Owner Name</span>
                  <span style={{ fontWeight: 700 }}>{connectionDetails.currentOwner}</span>
                </div>
                <div>
                  <span style={{ color: 'var(--text-muted)', display: 'block', fontSize: 12 }}>Connection Number</span>
                  <span style={{ fontWeight: 700 }}>{connectionDetails.connectionNo || 'PENDING APPROVAL'}</span>
                </div>
                <div>
                  <span style={{ color: 'var(--text-muted)', display: 'block', fontSize: 12 }}>Address</span>
                ...
                <div>
                  <span style={{ color: 'var(--text-muted)', display: 'block', fontSize: 12 }}>Usage Category</span>
                  <span>{connectionDetails.currentUsage}</span>
                </div>
                <div>
                  <span style={{ color: 'var(--text-muted)', display: 'block', fontSize: 12 }}>Status</span>
                  <span className={`badge ${statusBadgeClass(connectionDetails.connectionStatus)}`}>
                    {connectionDetails.connectionStatus}
                  </span>
                </div>
                <div>
                  <span style={{ color: 'var(--text-muted)', display: 'block', fontSize: 12 }}>Registered Date</span>
                  <span>{new Date(connectionDetails.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                </div>
              </div>

              {/* Service Transaction History */}
              <div>
                <h4 style={{ fontSize: 15, fontWeight: 700, marginBottom: 12, textTransform: 'uppercase' }}>Service Transaction Logs</h4>
                <div className="table-responsive" style={{ border: '2px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
                  <table className="table table-sm" style={{ margin: 0 }}>
                    ...
                    <tbody>
                      {(connectionDetails.records || []).length === 0 ? (
                        ...
                        (connectionDetails.records || []).map((r: any) => {
                          const totalPaid = (r.payments || []).reduce((sum: number, p: any) => sum + Number(p.amount), 0);
                          return (
                            <tr key={r.id}>
                              <td style={{ fontWeight: 600 }}>{WATER_SERVICE_TYPE_LABELS[r.serviceType] || r.serviceType}</td>
                              <td>{r.applicationTokenNo || '—'}</td>
                              <td>{r.dateOfService}</td>
                              <td style={{ textAlign: 'right' }}>₹{Number(r.officialFee).toLocaleString('en-IN')}</td>
                              <td style={{ textAlign: 'right' }}>₹{Number(r.serviceFee).toLocaleString('en-IN')}</td>
                              <td style={{ textAlign: 'right', fontWeight: 600 }}>₹{Number(r.amountCharged).toLocaleString('en-IN')}</td>
                              <td style={{ textAlign: 'right', fontWeight: 600, color: 'var(--success)' }}>₹{totalPaid.toLocaleString('en-IN')}</td>
                            </tr>
                          );
                        })
                      )}
            ...
            <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--danger)' }}>Failed to load profile details.</div>
          )}
        </Modal>
      )}
    </div>
