    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 2 }}>{today} — Welcome back, {user?.name}</div>
        </div>
      </div>
        ...
        <>
          <div className="grid-4" style={{ marginBottom: '1.5rem', gap: 12 }}>
            {[
              ...
              { label: 'Birth/Death earnings', value: `₹${data.birthDeathEarnings.toLocaleString('en-IN')}` },
              ...
              <div className="metric-card" key={label} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '1.25rem 1rem' }}>
          ...
          <div className="grid-2">
            <div className="card">
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                <button className="btn btn-primary" onClick={() => setFilterParams({ from: from || undefined, to: to || undefined })}>Calculate</button>
                <button className="btn" onClick={() => { setFrom(''); setTo(''); setFilterParams({}); }}>Clear</button>
              </div>
              {(filterParams.from || filterParams.to) && (
                <div style={{ marginTop: 14, fontSize: 13 }}>
                  <div className="price-row strong" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                    <span>Affidavits (Gross)</span><b>₹{data.affidavitGrossEarnings.toLocaleString('en-IN')}</b>
                  </div>
                  <div className="price-row strong" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                    <span>Affidavits (Net)</span><b>₹{data.affidavitNetEarnings.toLocaleString('en-IN')}</b>
                  </div>
                  <div className="price-row strong" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                    <span>Marriages</span><b>₹{data.marriageEarnings.toLocaleString('en-IN')}</b>
                  </div>
                  <div className="price-row strong" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                    <span>Birth/Death Cert.</span><b>₹{data.birthDeathEarnings.toLocaleString('en-IN')}</b>
                  </div>
                  <hr className="divider" />
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 500, marginBottom: 4 }}>
                    <span>Total Earnings (Gross)</span><span style={{ fontSize: 16 }}>₹{data.totalEarnings.toLocaleString('en-IN')}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 500 }}>
                    <span>Total Earnings (Net)</span><span style={{ fontSize: 16 }}>₹{data.totalNetEarnings.toLocaleString('en-IN')}</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="card">
              ...
              <div className="section-label">Certificates by type</div>
              <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {[
                  ...
                  <span key={key} className="badge badge-blue">{label}: {data.breakdown.byAct[key] || 0}</span>
                ))}
              </div>
            </div>
          </div>
