...
export class AuthService {
  private readonly registrationChallenges = new Map<string, { challenge: string; userId: string }>();
  private readonly authenticationChallenges = new Map<string, { challenge: string }>();
  ...
  async getRegisterOptions(userId: string) {
    ...
    const userPasskeys = await this.passkeyRepository.find({ where: { user: { id: userId } } });
    ...
    this.registrationChallenges.set(sessionId, { challenge: options.challenge, userId });
    setTimeout(() => this.registrationChallenges.delete(sessionId), 5 * 60 * 1000);
  ...
  async verifyRegister(sessionId: string, body: any) {
    ...
    if (!savedChallenge) throw new UnauthorizedException('Registration session expired or invalid');
    ...
    try {
      ...
      if (clientData.origin && clientData.origin.startsWith('android:apk-key-hash:')) {
    ...
    console.log('[Passkey Register] Storing Credential ID:', base64UrlCredentialID);
  ...
  async getLoginOptions() {
    ...
    const sessionId = Math.random().toString(36).substring(2, 15);
    this.authenticationChallenges.set(sessionId, { challenge: options.challenge });
    setTimeout(() => this.authenticationChallenges.delete(sessionId), 5 * 60 * 1000);
  ...
  async verifyLogin(sessionId: string, body: any) {
    ...
    if (!savedChallenge) throw new UnauthorizedException('Authentication session expired or invalid');
    ...
    console.log('[Passkey Login] Available credential IDs in DB:', allPasskeys.map(pk => pk.credentialID));
    ...
    if (!passkey.user || !passkey.user.isActive) throw new UnauthorizedException('User account is disabled');
    ...
    const payload = { sub: passkey.user.id, email: passkey.user.email, role: passkey.user.role };
    const token = this.jwtService.sign(payload);
